...
function render_content($content) {
	$links = preg_match_all("@[\[]{2}.+[\]]{2}@", $content, $matches);
			...
			elseif (substr(trim($alink), 0, 10) == '[[imgicon:') {
				...
				if (mysql_num_rows($result) > 0) {
					while($row = mysql_fetch_object($result)) {
						...
						if (array_key_exists(2,$data))
							$picture .= '</a>';
						$picture .= '</div></div><br class="clear" />';
						array_push($replace, $picture);
					}
				}
        else
...
function printadminmenu() {
  global $role;
  if ($role == 1) {
    echo '<h4>Site administration:</h4>';
    echo '<h4>Resources</h4>';
    echo '<ul>';
      echo '<li><a href="/administration?tool=pictures">Manage pictures</a>';
      echo '<li><a href="/administration?tool=files">Manage download files</a>';
      echo '<li><a href="/link_up">Publish Link-Up articles</a>';
    ...
    echo '<ul>';
      ...
      echo '<li><a href="/youth_programmes">Youth Activities</a>';
