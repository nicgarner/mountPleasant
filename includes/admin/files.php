<?php
  ...
  if ($action == 'd' && $confirm == 't') {
    ...
    echo '<meta http-equiv="refresh" content="0;url='.$_SERVER['REDIRECT_URL'].'?tool='.$tool.'&c=d">';
  }
  if ($_POST['submit']) {
    if ($_POST['submit'] == 'Upload file')
    {
      ...
      if ($upload_error == 0) {
        ...
        $errors['upload'] .= 'Contact <a href="mailto:webmaster@mountpleasantchurch.com">webmaster@mountpleasantchurch.com</a> for help.';
      }
      ...
      if ($errors == null) {
        ...
        {
          echo '<meta http-equiv="refresh" content="0;url='.$_SERVER['REDIRECT_URL'].'?tool='.$tool.'&id='.$id.'&a=u">';
          die();
        }
        else
          $errors['move_upload'] = 'There was an unknown problem uploading the file. Please try again, or contact <a href="mailto:webmaster@mountpleasantchurch.com">webmaster@mountpleasantchurch.com</a> for help.';
      }
    }
  }
...
<p>Here you can see files that have been uploaded to the site, and upload new ones. You can create download
   links to uploaded files with short pieces of code, click 'Edit details' to see how.</p>
...
<br class="clear" />
  ...
  ?>
  <form enctype="multipart/form-data" action="<?= $_SERVER['REDIRECT_URL'].'?tool='.$tool ?>" method="post" 
  ...
  <form enctype="multipart/form-data" action="<?= $_SERVER['REDIRECT_URL'].'?tool='.$tool ?>" 
...
if (mysql_num_rows($result) > 0) {
  ...
  while($row = mysql_fetch_object($result)) {
    ...
    echo '"><div class="image" style="padding:25px 10px; width:25px; height:30px;">';
    ...
    echo '<p><a href="'.$_SERVER['REQUEST_URI'].'&id='.$row->file_id.'">Edit details</a><br/>';
    echo '<a href="'.$_SERVER['REQUEST_URI'].'&id='.$row->file_id.'&a=d">Delete file</a></p>';
    echo '</div></div>';
    $box++;
  }
  echo '</div>';
...
<?php
  ...
  if (!$no_record) { ?>
    ...
    <div class="calendar"><div class="options">
      <div class="selected"><a href="<?= $_SERVER['REDIRECT_URL'].'?tool='.$tool; ?>">Back to<br/>files index</a></div>
    </div></div>
    <?
      ...
      <p style="margin:0"><a href="'.$_SERVER['REQUEST_URI'].'&c=t">Yes, delete the picture</a> |
      <a href="'.$_SERVER['REDIRECT_URL'].'?tool='.$tool.'">No, cancel!</a></p></div>';
      ...
      echo '<br class="clear"/>';
      echo '<br class="clear"/>';
      echo '<p>To generate the above link in a page, type <tt>[[download:'.$name.'.'.$filetype.']]</tt></p>';
