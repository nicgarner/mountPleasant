<?php 
	...
	if (mysql_num_rows($result) > 0) {
		echo '<div class="banner_ads" id="banner_ads" onmouseover="stopSwitchHeadlines()" onmouseout="resumeSwitchHeadlines('.mysql_num_rows($result).',showingad,'.$delay.')">';
		echo '<div class="left"></div>';
		echo '<div class= "middle" style="width:700px;">';
		$i = 1;
		while($row = mysql_fetch_object($result)) {
			...
			echo '>';
			echo '<div class="banner_ad">';
			if (! $row->image == NULL) {
				if (! $row->shortname == NULL)
					echo '<a href="'.$PHP_SELF.$row->shortname.'">';
				echo '<img style="width:281px;" src="images/graphics/news/_'.$row->image.'"';
